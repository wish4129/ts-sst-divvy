...
### 2026-06-19
- **[SHENNONG] normalizeReport() for raw/recommendation AI report formats, widen gitignore pattern** — StockDetail.tsx now normalizes raw (string) and recommendation (object-with-raw) AI report formats to a consistent 5-section structure; gitignore widened to data/divvy.db* pattern (754d0eb1) [SHENNONG] [Internal]
- **[SAFE] Commit dirty code: 5-section AI report labels, trailing-slash URLs, generated artifact refresh** — StockDetail.tsx now shows AI_REPORT_LABELS for 5 sections (overview, risk_assessment, financial_health, growth_prospects) with preview/gated section split (cosmetic), generatedAt date display, and trailing-slash canonical URLs. generate_sitemap.py and generate_stock_meta.py now emit trailing slash on /stock/{code}/ URLs. Artifact refresh: kronos_forecast.json (148 entries), macro data, stocks.ts. Gitignore widened to data/divvy.db.bak*. (82f1aa7f)
- **[SAFE] Fix: remove tracked DB backup from index, widen gitignore pattern to data/divvy.db*** — removes accidentally committed DB backup file, widens pattern to cover all data/divvy.db* variants (0328a88e) [Internal]
