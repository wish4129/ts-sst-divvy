"""Industry-specific stock scorer — reads industry_matrix.json + stocks data."""
import json
from pathlib import Path
from datetime import datetime

ROOT = Path(__file__).resolve().parent.parent
MATRIX_PATH = ROOT / "data" / "industry_matrix.json"
...
# ── Stock data (port from web/src/data/stocks.ts — same structure) ──
# For now read from portfolios.json + screener_results.json as source
PORTFOLIOS = json.loads((ROOT / "scripts" / "portfolios.json").read_text())
SCREENER = json.loads((ROOT / "data" / "screener_results.json").read_text()) if (ROOT / "data" / "screener_results.json").exists() else {"new_candidates": []}

# ── Financial data lookup from screener ──
FINANCIALS = {}
for c in SCREENER.get("new_candidates", []):
    FINANCIALS[c["code"]] = c  # screener uses short codes like "MAGNI"

# Map short codes → ticker codes
SHORT_TO_TICKER = {}
for name, info in PORTFOLIOS["stocks"].items():
    SHORT_TO_TICKER[name] = info["code"]
    SHORT_TO_TICKER[name.lower()] = info["code"]  # case-insensitive

def get_financial(stock_code, factor_name):
    """Map factor name to available financial data."""
    # Try direct code match first, then short name reverse lookup
    f = FINANCIALS.get(stock_code)
    if not f:
        # Reverse lookup: find short name for this ticker code
        for short, ticker in SHORT_TO_TICKER.items():
            if ticker == stock_code and short in FINANCIALS:
                f = FINANCIALS[short]
                break
    if not f:
        f = {}
    ...
    mapping = {
        "dividend_yield": f.get("dividendYield", 0),
        "pe_ratio": f.get("pe", 99),
        "pb_ratio": f.get("pb", 99),
        "roe": f.get("roe", 0),
        "de_ratio": f.get("debtToEquity", 99),
        "revenue_growth_yoy": f.get("revenueGrowthYoY", 0) if "revenueGrowthYoY" in f else 5.0,
        "gross_margin": 25.0,
        "market_cap": f.get("marketCap", 0),
    }
    return mapping.get(factor_name, None)


def score_factor(factor_name, value, weight, higher_better):
    """Score a single factor on 0-100 scale based on Bursa context.

    Uses reasonable Malaysia-market benchmarks. Higher score = better.
    Expected ranges below are for MY stocks specifically."""
    if value is None:
        return 0
...
def score_stock(stock_code, stock_name, industry, financials_override=None):
    """Compute industry-specific score for a stock."""
    factors = INDUSTRIES.get(industry)
    if not factors:
        factors = {"factors": [
            {"name": "dividend_yield", "label": "Dividend Yield", "weight": FALLBACK["dividend"], "higher_better": True, "source": "market"},
            {"name": "revenue_growth_yoy", "label": "Revenue Growth YoY", "weight": FALLBACK["growth"], "higher_better": True, "source": "quarterly"},
            {"name": "roe", "label": "Return on Equity", "weight": FALLBACK["quality"], "higher_better": True, "source": "quarterly"},
            {"name": "de_ratio", "label": "Debt-to-Equity", "weight": FALLBACK["risk"], "higher_better": False, "source": "quarterly"},
        ]}
    ...
    for f in factors["factors"]:
        ...
        composite += weighted
        breakdown[f["name"]] = {"value": value, "raw": round(raw, 1), "weighted": round(weighted, 1)}
    ...
    return {
        ...
        "breakdown": breakdown,
        "macro_sensitivity": factors.get("macro_sensitivity", {}),
    }


# ── Score all stocks ──
print("=" * 60)
print("Divvy Industry-Specific Stock Scores")
print("=" * 60)
...
# Print comparison
print(f"\n{'Stock':10s} {'Industry':20s} {'New':>6s} {'Old':>6s} {'Δ':>6s}")
print("-" * 55)

# Load old scores for comparison (short names → composite)
STOCK_MAP = {}
for name, info in PORTFOLIOS["stocks"].items():
    STOCK_MAP[info["code"]] = name

# Old scores from web/src/data/stocks.ts
OLD_SCORES = {}
for code, old in [
...
for r in results:
    ...
    print(f"{r['code']:10s} {r['industry']:20s} {r['composite']:6.1f} {old:6d} {d:>6s}")
...
Path(ROOT / "data" / "stock_scores.json").write_text(json.dumps(OUTPUT, indent=2))
...
print(f"{'─'*40}")
for r in results[:3]:
    print(f"\n{r['code']} ({r['industry']}) — composite: {r['composite']:.1f}")
    for factor_name, b in r["breakdown"].items():
        v = b['value'] if b['value'] is not None else 'N/A'
        print(f"  {factor_name:20s} value={v:>8} → raw={b['raw']:5.1f} × {b['weighted']/b['raw']*100 if b['raw'] > 0 else 0:.0f}% = {b['weighted']:.1f}")
